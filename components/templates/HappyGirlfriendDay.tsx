...
const TypewriterText = ({ text, onComplete }: { text: string, onComplete?: () => void }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setDisplayedText(text.slice(0, iRef.current + 1));
      iRef.current++;
...
const DesktopScene = ({ name, nextScene }: { name: string, nextScene: (s: Scene) => void }) => (
...
const Scene1 = ({ name, nextScene, playSound }: { name: string, nextScene: (s: Scene) => void, playSound: (e: SoundEffect) => void }) => {
...
const Scene2 = ({ lovePoints, setLovePoints, nextScene }: { lovePoints: number, setLovePoints: React.Dispatch<React.SetStateAction<number>>, nextScene: (s: Scene) => void }) => {
  useEffect(() => {
    const timer = setTimeout(() => setLovePoints(100), 1000);
...
const Scene3 = ({ nextScene, playSound }: { nextScene: (s: Scene) => void, playSound: (e: SoundEffect) => void }) => {
    ...
    <PixelWindow title="Question.exe">
      <div className="flex flex-col items-center gap-6 min-h-[300px]">
        ...
        <div className="bg-white border-4 border-[#2B2B2B] p-4 w-full min-h-[100px]">
          {step === 0 ? (
            <TypewriterText key="q" text="Do you know how amazing you are?" />
          ) : (
            <TypewriterText key="a" text={response} />
          )}
        </div>
...
const Scene5 = ({ openedItems, setOpenedItems, nextScene, playSound }: { openedItems: string[], setOpenedItems: React.Dispatch<React.SetStateAction<string[]>>, nextScene: (s: Scene) => void, playSound: (e: SoundEffect) => void }) => {
  const items = [
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-sm mt-4">
          {items.map(item => (
            <motion.div
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => handleOpen(item.id, item.target as Scene)}
...
const SceneFlowers = ({ nextScene }: { nextScene: (s: Scene) => void }) => (
...
const SceneHugs = ({ nextScene }: { nextScene: (s: Scene) => void }) => (
...
const Scene8 = ({ letter_text, nextScene, playSound }: { letter_text: string, nextScene: (s: Scene) => void, playSound: (e: SoundEffect) => void }) => {
...
const FinalScene = ({ playSound }: { playSound: (e: SoundEffect) => void }) => {
  ...
  return (
    <div className="w-full min-h-screen bg-[#FFF8E7] flex flex-col items-center justify-center overflow-hidden selection:bg-[#95BBEA]">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {scene === "desktop" && <DesktopScene name={name} nextScene={nextScene} />}
          {scene === "loading" && <LoadingScene />}
          {scene === "scene1" && <Scene1 name={name} nextScene={nextScene} playSound={playSound} />}
          {scene === "scene2" && <Scene2 lovePoints={lovePoints} setLovePoints={setLovePoints} nextScene={nextScene} />}
          {scene === "scene3" && <Scene3 nextScene={nextScene} playSound={playSound} />}
          {scene === "scene5" && <Scene5 openedItems={openedItems} setOpenedItems={setOpenedItems} nextScene={nextScene} playSound={playSound} />}
          {scene === "scene_flowers" && <SceneFlowers nextScene={nextScene} />}
          {scene === "scene_hugs" && <SceneHugs nextScene={nextScene} />}
          {scene === "scene8" && <Scene8 letter_text={letter_text} nextScene={nextScene} playSound={playSound} />}
          {scene === "final" && <FinalScene playSound={playSound} />}
        </motion.div>
      </AnimatePresence>
    </div>
  );
